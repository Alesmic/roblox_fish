...
Configuration.Currency = {
	...
	DataStoreName = "PlayerCurrency_AquariumV1" -- 修改名字以重置数据，避免旧数据冲突
}
...
Configuration.MaxFishInventory = {
	Enabled = true,
	MaxFish = 200,
	WarningMessage = "背包已满！请先出售鱼获。"
}

-- ============================================
-- 2. 区域设置 (ZONE SETTINGS)
-- ============================================
-- 名字必须对应 Workspace 中的 Part 名字 (例如 Workspace.FishingZones.Ocean)
Configuration.Zones = {
	["Default"] = {1, 2, 7, 11}, -- 默认鱼池 (找不到区域时)
	["Ocean"] = {3, 5, 6, 8, 9, 10, 13}, -- 海洋鱼类
	["Lake"] = {1, 2, 4, 12}, -- 湖泊鱼类
}
...
Configuration.Aquarium = {
	IncomePercentage = 0.2, -- 收益率 20% (0.2)
	IncomeRate = 1,         -- 每多少秒发放一次收益
	MaxOfflineTime = 43200, -- 离线收益最大时长 (秒), 这里是12小时
}
...
Configuration.ChatNotification = {
	Enabled = true,
	MessageFormat = "[系统] %s 钓到了一条 %s (%.1fkg) %s!",
	...
	RarityTiers = {
		...
		{threshold = 100, name = "Common", color = Color3.fromRGB(255, 255, 255)} -- 添加 Common 防止报错
	},
